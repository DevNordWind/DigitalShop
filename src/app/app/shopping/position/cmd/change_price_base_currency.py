...
class ChangePositionPriceBaseCurrencyCmd:
    id: UUID
    currency: Currency


class ChangePositionPriceBaseCurrency:
    def __init__(
        self,
    ...
    async def __call__(
        self,
        cmd: ChangePositionPriceBaseCurrencyCmd,
    ) -> None:
        ...
        position.change_price_base_currency(
            now=self._clock.now(), currency=cmd.currency
        )
