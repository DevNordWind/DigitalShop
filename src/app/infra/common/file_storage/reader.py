...
from app.infra.common.file_storage.mapper import LocalFileStorageErrorMapper
...
class FileSystemReader(FileStorageReader):
    ...
    async def read(self, key: FileKey) -> AsyncIterator[bytes]:
        ...
        async def _reader() -> AsyncIterator[bytes]:
            try:
                ...
                raise FileStorageFileNotFoundError(key.value) from e
    ...
    async def resolve(self, key: FileKey) -> ResolvedKey:
        ...
        if not await asyncio.to_thread(path.exists):
            raise FileStorageFileNotFoundError(key.value)
    ...
    def _resolve(self, key: FileKey) -> Path:
        ...
        if not path.is_relative_to(self._base_path):
            raise FileKeyInvalidError(invalid_key=key.value)
