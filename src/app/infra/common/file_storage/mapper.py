...
class LocalFileStorageErrorMapper:
    MAPPING: frozendict[type[Exception], type[FileStorageError]] = frozendict(
        {
            FileNotFoundError: FileStorageFileNotFoundError,
            PermissionError: FilePermissionDeniedError,
    ...
    def map_and_raise(cls, orig_exc: Exception) -> Never:
        ...
        raise (mapped() if mapped is not None else FileStorageError()) from orig_exc
