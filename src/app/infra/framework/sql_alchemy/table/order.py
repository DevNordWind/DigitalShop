...
from app.domain.order.value_object import AppliedCoupon, PaymentSource
...
order_table: Table = Table(
    ...
    Column(
        "customer_id", UserIdType, ForeignKey("User.id"), nullable=False, index=True
    ),
    Column(
        "sub_total_amount",
        NUMERIC(19, 4),
    ...
    Column("items_amount", ItemsAmountType, nullable=False),
...
def compose_payment_source(
    ...
    return PaymentSource(
        payment_id=PaymentId(value=payment_id) if payment_id is not None else None,
        type=tp,
    )
...
def compose_applied_coupon(
    ...
    if coupon_id is None or discount_amount is None or discount_currency is None:
        return None
