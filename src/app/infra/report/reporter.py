...
class SqlAReporter(Reporter):
    ...
    async def _report_sales(self, period: TimePeriod | None) -> SalesReport:
        ...
        if period is not None:
            stmt = stmt.where(
                order_table.c.confirmed_at.between(period.from_date, period.to_date),
            )
        stmt = stmt.group_by(order_table.c.total_currency)

        rows = (await self._session.execute(stmt)).all()
        total_count, amount = self._aggregate_money(rows, "total_currency", "amount")
    ...
    async def _report_top_ups(self, period: TimePeriod | None) -> TopUpsReport:
        ...
        if period is not None:
            stmt = stmt.where(
                payment_table.c.created_at.between(period.from_date, period.to_date),
            )
    ...
    async def _report_products(self, period: TimePeriod | None) -> ProductsReport:
        fixed_items_stmt = select(func.count(fixed_item_table.c.id))
        stock_items_stmt = select(func.count(stock_item_table.c.id))
        positions_stmt = select(func.count(position_table.c.id))
        categories_stmt = select(func.count(category_table.c.id))

        if period is not None:
            fixed_items_stmt = fixed_items_stmt.where(
            ...
            categories_stmt = categories_stmt.where(
                category_table.c.created_at.between(period.from_date, period.to_date)
            )

        stmt = select(
            stock_items_stmt.scalar_subquery().label("stock_items_count"),
            fixed_items_stmt.scalar_subquery().label("fixed_items_count"),
        ...
        return ProductsReport(
            items_count=(row.stock_items_count or 0) + (row.fixed_items_count or 0),
    ...
    ) -> tuple[int, dict[Currency, Money]]:
        total_count = sum(row.count for row in rows)

        amount: dict[Currency, Money] = {
            getattr(row, currency_field): Money(
