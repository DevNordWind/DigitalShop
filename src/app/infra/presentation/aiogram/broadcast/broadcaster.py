...
type UpdateProgressTaskType = AsyncTaskiqDecoratedTask[[UUID, BroadcastReporting], None]
...
class TelegramBroadcasterImpl(TelegramBroadcaster):
    ...
    def __init__(
        ...
        update_task = priority_broker.find_task(task_name=UPDATE_PROGRESS_TASK_NAME)
    ...
    async def broadcast(self, request: BroadcastRequest) -> None:
        ...
        langs: set[Language] = {ctx.lang for ctx in contexts if ctx.lang is not None}
        markups: dict[Language, InlineKeyboardMarkup] | None = None
        if request.buttons:
            markups = self._kb_builder.build_many(langs=langs, buttons=request.buttons)
    ...
    async def _start_progress(
        ...
        try:
            start_msg_id = (
                await self._progress_msg.start(reporting=request.reporting, total=total)
            ).message_id
