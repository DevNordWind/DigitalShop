...
from app.infra.presentation.aiogram.broadcast.const import (
    SEND_MSG_TASK_NAME,
    UPDATE_PROGRESS_TASK_NAME,
)
from app.infra.presentation.aiogram.broadcast.dto import (
    BroadcastProgress,
    MessageResult,
)
...
from app.presentation.aiogram.port.broadcast import BroadcastMedia
from app.presentation.aiogram.port.broadcast.dto import BroadcastReporting
...
async def update_progress(
    ...
    try:
        msg = await message.update(reporting=reporting, progress=current_progress)
...
async def send_message(
    ...
    try:
        ...
        logger.info("Sent message to chat id: %s", chat_id)
        ...
        if retries >= SEND_MSG_MAX_RETRIES:
            ...
            return result
        logger.error(
            "Exception: %s, retries: %s, max: %s", e, retries, SEND_MSG_MAX_RETRIES
        )
        raise
...
def _resolve_send_method(
    ...
    if media is None:
        return bot.send_message(chat_id=chat_id, text=text, reply_markup=markup)
