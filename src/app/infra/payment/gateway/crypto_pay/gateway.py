...
from app.app.payment.port import PaymentMethodGateway, PaymentMethodGatewayError
...
from app.infra.payment.gateway.crypto_pay.mapper import CryptoPayInvoiceMapper
...
class CryptoPayPaymentGateway(PaymentMethodGateway):
    ...
    async def get(self, data: GetInvoice) -> Invoice:
        try:
            ...
            raise InvalidInvoiceIdError from e
        ...
        try:
            ...
            if isinstance(crypto_pay_invoice, list):
                try:
                    ...
                    raise InvoiceNotFoundError from e
