...
from app.infra.framework.sql_alchemy.table.telegram_context import (
...
class TelegramContextGatewayImpl(TelegramContextGateway):
    ...
    @override
    async def get_data(self, telegram_id: TelegramId) -> TelegramContextData | None:
        stmt = (
    ...
    async def get_admins(
        ...
        stmt = (
            ...
            .join(user_table, telegram_context_table.c.user_id == user_table.c.id)
        )
    ...
    async def get_all_by_langs(
        ...
        stmt = select(TelegramContext).where(telegram_context_table.c.lang.in_(langs))
        if is_active is not None:
            stmt = stmt.where(
                telegram_context_table.c.is_active.is_(is_active),
            )
