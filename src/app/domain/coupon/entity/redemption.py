...
from app.domain.coupon.exception.redemption import (
    CouponRedemptionCancellationForbiddenError,
    CouponRedemptionConfirmationForbiddenError,
)
...
class CouponRedemption:
    ...
    def confirm(self, now: datetime) -> None:
        if self.status != CouponRedemptionStatus.RESERVED:
            raise CouponRedemptionConfirmationForbiddenError
    ...
    def cancel(self, now: datetime) -> None:
        if self.status != CouponRedemptionStatus.RESERVED:
            raise CouponRedemptionCancellationForbiddenError
