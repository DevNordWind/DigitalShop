...
from app.domain.common.coefficient.exception import (
...
class Coefficient:
    ...
    def __post_init__(self) -> None:
        if self.value < MIN_COEFFICIENT:
            raise CoefficientTooSmallError(min_coefficient=MIN_COEFFICIENT)

        if self.value > MAX_COEFFICIENT:
            raise CoefficientTooBigError(max_coefficient=MAX_COEFFICIENT)
