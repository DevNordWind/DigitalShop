...
from app.presentation.aiogram.dialog.user.profile.order.ctx import ORDER_SCROLL
from app.presentation.aiogram.dialog.user.profile.order.getter import (
    filters_getter,
    order_getter,
    orders_getter,
)
from app.presentation.aiogram.state import OrdersState
...
filters = Window(
    ...
    Button(
        ...
        id="sorting_order",
        on_click=on_sorting_order,  # type: ignore[bad-argument-type]
    ),
    Group(
        Select(
...
order = Window(
    ...
    Button(
        ...
        on_click=on_upload_items,  # type: ignore[bad-argument-type]
    ),
    SwitchTo(GetText("inl-ui.back"), id="back", state=OrdersState.orders),
