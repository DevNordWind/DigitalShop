...
from app.presentation.aiogram.dialog.admin.payment_settings.ctx import (
    CTX_KEY,
    PaymentSettingsCtx,
)
from app.presentation.aiogram.setting.payment.cmd import (
    SwitchPaymentSettingStatus,
    SwitchPaymentSettingStatusCmd,
)
from app.presentation.aiogram.state.admin import PaymentSettingsState
...
async def on_switch_status(
    ...
    return await handler(
        SwitchPaymentSettingStatusCmd(method=PaymentMethod(ctx.current_method)),
    )
