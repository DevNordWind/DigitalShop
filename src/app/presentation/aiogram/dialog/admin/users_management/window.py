...
from app.presentation.aiogram.state import BroadcastState, UsersManagementState
...
find = Window(
    ...
    SwitchTo(GetText("inl-ui.back"), id="back", state=UsersManagementState.users),
    state=UsersManagementState.find,
)
...
user = Window(
    ...
    Button(
        ...
        id="promote_to_admin",
        on_click=on_promote_to_admin,  # type: ignore[bad-argument-type]
        when=(F["role"] == UserRole.USER) & (F["user_role"] >= UserRole.SUPER_ADMIN),
    ),
    Button(
        GetText("users-management-user.demote-to-user-btn"),
        id="demote_to_user",
        on_click=on_demote_to_user,  # type: ignore[bad-argument-type]
        when=(F["role"] == UserRole.ADMIN) & (F["user_role"] >= UserRole.SUPER_ADMIN),
    ),
    SwitchTo(
    ...
    Button(
        ...
        id="start_orders",
        on_click=on_start_orders,  # type: ignore[bad-argument-type]
    ),
    SwitchTo(GetText("inl-ui.back"), id="back", state=UsersManagementState.find),
    getter=user_getter,
...
input_top_up_amount = Window(
    ...
    SwitchTo(GetText("inl-ui.back"), id="back", state=UsersManagementState.user),
    state=UsersManagementState.input_top_up_amount,
