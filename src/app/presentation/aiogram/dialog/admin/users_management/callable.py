...
from app.app.wallet.cmd import TopUpWalletManually, TopUpWalletManuallyCmd
from app.domain.common.money import Currency
from app.domain.user.enums import UserRole
from app.presentation.aiogram.dialog.admin.users_management.ctx import (
    CTX_KEY,
    UsersManagementCtx,
)
from app.presentation.aiogram.port import Text
from app.presentation.aiogram.state import OrdersState, UsersManagementState
...
async def on_input_user_identifier(
    ...
    handler: FromDishka[GetUserProfile],
) -> None:
    profile: UserProfileDTO = await handler(
        GetUserProfileQuery(target_identifier=identifier)
    )
    await event.delete()
...
async def on_demote_to_user(
    ...
    await handler(
        AssignUserRoleCmd(target_user_id=ctx.current_user_id, target_role=UserRole.USER)
    )
...
async def on_input_top_amount(
    ...
    if ctx.current_top_up_currency is None:
        return await event.reply(t("users-management-top-up.unselected-currency"))

    await handler(
        TopUpWalletManuallyCmd(
            amount=MoneyDTO(amount=amount, currency=ctx.current_top_up_currency),
