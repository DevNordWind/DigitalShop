...
from app.app.common.dto.period import TimePeriodMapper
from app.app.report.dto.general import ConvertedGeneralReportDTO
from app.app.report.query import (
...
from app.presentation.aiogram.dialog.admin.statistic.ctx import (
    CTX_KEY,
    AdminStatisticCtx,
    PeriodUnit,
)
...
async def statistic_getter(
    ...
    **_: Any,
    ...
    report: ConvertedGeneralReportDTO = await query_handler(
        GetConvertedGeneralReportQuery(
            period=TimePeriodMapper.to_dto(src=time_period) if time_period else None,
            convert_to=ctx.convert_to,
        )
    )
    ...
    buttons.extend(
        [
            PeriodUnitButton(
                unit=unit,
                is_current=ctx.period_unit == unit and ctx.custom_period is None,
            )
...
async def convert_getter(
    ...
    **_: Any,
    ...
    return {
        "buttons": [
            CurrencyButton(currency=currency, is_current=ctx.convert_to == currency)
