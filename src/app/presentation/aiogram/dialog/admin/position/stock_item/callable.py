...
from app.presentation.aiogram.dialog.admin.position.stock_item.ctx import (
    CTX_KEY,
    ItemsFilters,
...
async def on_filter_order(
    ...
    ctx.filters.order = (
        SortingOrder.DESC if ctx.filters.order == SortingOrder.ASC else SortingOrder.ASC
    )
    dialog_manager.dialog_data[CTX_KEY] = retort.dump(ctx)
...
async def on_add_item_text(
    ...
    items_ids = await handler(
        AddPositionItemsCmd(
            id=ctx.position_id, contents=[ItemContentDTO(value=item) for item in items]
        ),
    )
    await event.reply(
        text=t(
...
async def on_replace_item_content_text(
    ...
    await handler(
        ReplacePositionItemCmd(
            item_id=ctx.current_item_id, new_content=ItemContentDTO(value=new_content)
        ),
    )
