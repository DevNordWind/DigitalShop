...
async def item_getter(
    ...
    **_: Any,
    ...
    item: ItemDTO = await query_handler(
        GetPositionItemQuery(item_id=ctx.current_item_id),
    )
    if not isinstance(item, StockItemDTO):
        return {}
