...
from app.app.shopping.position.cmd import (
    ...
    ArchiveAllPositionsByCategoryCmd,
    ArchivePosition,
    ArchivePositionCmd,
    DeleteAllPositionsByCategory,
    DeleteAllPositionsByCategoryCmd,
    DeletePosition,
    DeletePositionCmd,
    RecoverPosition,
    RecoverPositionCmd,
...
from app.app.shopping.position.query import GetPosition, GetPositionQuery
...
from app.presentation.aiogram.dialog.admin.position.ctx import (
    CTX_KEY,
...
async def on_archive_all_positions(
    ...
    dialog_manager: DialogManager,
    retort: FromDishka[Retort],
    handler: FromDishka[ArchiveAllPositionsByCategory],
) -> None:
    ctx: AdminPositionCtx = retort.load(
        dialog_manager.dialog_data[CTX_KEY],
        AdminPositionCtx,
    ...
    await handler(
        ArchiveAllPositionsByCategoryCmd(category_id=ctx.current_category_id),
    )
...
async def on_delete_all_positions(
    ...
    dialog_manager: DialogManager,
    retort: FromDishka[Retort],
    handler: FromDishka[DeleteAllPositionsByCategory],
) -> None:
    ctx: AdminPositionCtx = retort.load(
        dialog_manager.dialog_data[CTX_KEY],
        AdminPositionCtx,
    ...
    await handler(
        DeleteAllPositionsByCategoryCmd(category_id=ctx.current_category_id),
    )
