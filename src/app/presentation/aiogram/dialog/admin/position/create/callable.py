...
from app.app.shopping.position.cmd import CreatePosition, CreatePositionCmd
from app.app.shopping.position.dto.description.mapper import (
...
from app.infra.presentation.aiogram.mapper import FileDTOFactory
from app.presentation.aiogram.dialog.admin.position.create.ctx import (
    CTX_KEY,
    MEDIA_SCROLL,
    PositionCreationCtx,
)
from app.presentation.aiogram.setting.position.model import PositionSettings
from app.presentation.aiogram.util.mapper import (
...
async def on_confirm(
    ...
    position_media: list[FileKeyRawDTO] = [
        await file_factory.from_media_attachment(attachment) for attachment in ctx.media
    ]
    ...
    await handler(
        CreatePositionCmd(
            ...
            name=ctx.name,
            description=ctx.description if ctx.description.translations else None,
...
async def on_translate_name_to_others(
    ...
    ctx.name = LocalizedTextDTO(
        default_lang=settings.default_lang,
        translations=dict(translations.items()) | {settings.default_lang: source_text},
    )
    dialog_manager.dialog_data[CTX_KEY] = retort.dump(ctx)
