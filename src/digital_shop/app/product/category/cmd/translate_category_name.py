...
class TranslateCategoryNameToOthersCmd:
    id: UUID


class TranslateCategoryNameToOthers:
    def __init__(
        self,
    ...
    async def __call__(self, cmd: TranslateCategoryNameToOthersCmd) -> None:
        ...
        source_text: str = category.name.get_default()

        coroutines: dict[Language, Awaitable[str]] = {}
        for lang in Language:
            if lang != category.name.default_lang:
                coroutines[lang] = self._translator.translate(
                    source=category.name.default_lang,
                    target=lang,
                    text=source_text,
                )
        ...
        for lang, name in translations.items():
            category.set_name(lang=lang, name=name, now=now)
