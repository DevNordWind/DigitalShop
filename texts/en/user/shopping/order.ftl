...
user-shopping-order-line = { $is_last ->
    [False] ├ <b>{ $position_name }:</b> { $items_amount } pcs. <code>{ $total_price }{ currency.symbol }</code>
    *[True] └ <b>{ $position_name }:</b> { $items_amount } pcs. <code>{ $total_price }{ currency.symbol }</code>
}
